...
if "%1"=="installer" (
	if exist "%wixBinDir%" (
	  if exist "%~dp0\CmisSync.msi" del "%~dp0\CmisSync.msi"
		...
		"%wixBinDir%\light" -ext WixUIExtension -ext WixUtilExtension -ext "%wixBinDir%\WixNetFxExtension.dll" -cultures:en-us CmisSync.wixobj plugins.wixobj -droot="%~dp0\..\.." -dpluginsdir="%~dp0\..\..\bin\plugins"  -o CmisSync.msi 
