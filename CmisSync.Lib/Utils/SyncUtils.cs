...
namespace CmisSync.Lib
{
    ...
    public static class SyncUtils
    {
        ...
        /// Log.
        /// </summary>
        private static readonly ILog Logger = LogManager.GetLogger(typeof(SyncUtils));
        ...
        private static Regex conflicVersionRegex = new Regex("-conflict-version");
        ...
        private static Regex ignoredFilenamesRegex = new Regex(
            ...
            "|" + conflicVersionRegex.ToString() + // CmisSync conflict
            ")"
        );
        ...
        {
            return conflicVersionRegex.IsMatch(filename);
        }
        ...
        public static string SyncedFolderDefaultNameFromPath(string path)
        {
            String[] p = path.Split(Cmis.CmisPath.DirectorySeparatorChar);
            if (p.Length > 2 && p[1].Equals("Siti"))
            {
                if (p.Length == 4 && p[3].Equals("documentLibrary"))
                {
                    return p[2];
                }
                else
                {
                    if (p.Length > 3)
                    {
                        return p[2] + System.IO.Path.DirectorySeparatorChar + p[p.Length - 1];
                    }
                    else
                    {
                        return p[2];
