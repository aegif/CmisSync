    ...
    public partial class CmisRepo : RepoBase
    {
        ...
        public partial class SynchronizedFolder : IDisposable
        {
            ...
            private bool SyncDownloadFile(IDocument remoteDocument, string localFolder, IList<string> remoteFiles = null)
            {
                ...
                try
                {
                    if (syncItem.ExistsLocal())      // local path
                    {
                        ...
                        {
                            Logger.Info("Downloading file absent from database: " + syncItem.LocalPath);  // local path
            ...
            private bool DownloadFile(IDocument remoteDocument, string localFolder)
            {
                ...
                if (Utils.IsInvalidFileName(fileName))      // local
                {
                ...
                try
                {
                    ...
                    if (Directory.Exists(filepath))     // local path
                    {
                        Directory.Delete(filepath);     // local path
                    }
                    ...
                    if (success)
                    {
                        ...
                        if (File.Exists(filepath)) // Conflict // local path
                        {
                            // Rename local file with a conflict suffix.
                            string conflictFilename = Utils.CreateConflictFilename(filepath, repoinfo.User);        // local path
                            File.Move(filepath, conflictFilename);      // local path

                            // Remove the ".sync" suffix.
                            File.Move(tmpfilepath, filepath);       // local path
                            SetLastModifiedDate(remoteDocument, filepath, metadata);    // database query

                            // Warn user about conflict.
                            string lastModifiedBy = CmisUtils.GetProperty(remoteDocument, "cmis:lastModifiedBy");
                            string message =
                                String.Format("User {0} added a file named {1} at the same time as you.", lastModifiedBy, filepath)     // local path
                                + "\n\n"
                        ...
                        {
                            Logger.Debug(String.Format("Renaming temporary local download file {0} to {1}", tmpfilepath, filepath));    // local path
                            // Remove the ".sync" suffix.
                            File.Move(tmpfilepath, filepath);       // local path
                            SetLastModifiedDate(remoteDocument, filepath, metadata);        // local path
                        }


                        if (null != remoteDocument.CreationDate)
                        {
                            File.SetCreationTime(filepath, (DateTime)remoteDocument.CreationDate);      // local path
                        }
                        if (null != remoteDocument.LastModificationDate)
                        {
                            File.SetLastWriteTime(filepath, (DateTime)remoteDocument.LastModificationDate);     // local path
                        }
            ...
            private bool UploadFile(string filePath, IFolder remoteFolder)
            {
                ...
                Logger.Info("Uploading: " + filePath);      // local path
                ...
                try
                {
                    ...
                    properties.Add(PropertyIds.CreationDate, File.GetCreationTime(filePath));   // local path
                    properties.Add(PropertyIds.LastModificationDate, File.GetLastWriteTime(filePath));      // local path
                    ...
                    using (CryptoStream hashstream = new CryptoStream(file, hashAlg, CryptoStreamMode.Read))        // 
                    {
                        ContentStream contentStream = new ContentStream();
                        contentStream.FileName = fileName;
                        contentStream.MimeType = MimeType.GetMIMEType(fileName);        // local
